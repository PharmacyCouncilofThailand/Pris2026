          ...
          <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-end gap-6">
            <div>
              <h1 className="text-3xl md:text-5xl font-black text-white font-outfit uppercase tracking-tight mb-4">
                Submission <span className="text-gold">Status</span>
              </h1>
              <p className="text-slate-400 font-medium">
                Track and manage your abstract submissions for PRIS 2026.
              </p>
            </div>
            <Link 
      ...
      <section className="container mx-auto px-4 -mt-8 relative z-10 pb-20">
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
            <StatCard label="Total Submissions" value="3" icon={<FileText />} color="blue" />
            <StatCard label="Accepted" value="1" icon={<CheckCircle />} color="emerald" />
            <StatCard label="Under Review" value="1" icon={<Clock />} color="amber" />
            <StatCard label="Revision Needed" value="1" icon={<BarChart3 />} color="indigo" />
          </div>
          ...
          <div className="bg-white rounded-[2rem] shadow-xl shadow-slate-200 border border-slate-100 overflow-hidden">
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-white">
                    <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[2px]">{abstractStatusLabels.table.id}</th>
                    <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[2px]">{abstractStatusLabels.table.title}</th>
                    <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[2px]">{abstractStatusLabels.table.date}</th>
                    <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[2px]">{abstractStatusLabels.table.status}</th>
                    <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[2px] text-right">{abstractStatusLabels.table.actions}</th>
                  </tr>
                </thead>
            ...
            <div className="p-8 text-center bg-slate-50 group">
              <p className="text-xs font-bold text-slate-400 uppercase tracking-[3px]">
                Showing {mockSubmissions.length} of {mockSubmissions.length} Submissions
              </p>
            </div>
          </div>
        </div>
