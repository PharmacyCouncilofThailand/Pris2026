...
export default function AbstractTracker() {
  const { token } = useAuth();
  const t = useTranslations("abstractTracker");
  ...
  return (
    <div className="space-y-5">
      {abstracts.map((item) => {
        ...
        const sections = [
          { label: t("background"), value: item.background },
          { label: t("objective"), value: item.objective },
          { label: t("methods"), value: item.methods },
          { label: t("results"), value: item.results },
          { label: t("conclusion"), value: item.conclusion },
        ];
                  ...
                  <div className="mt-8 pt-6 border-t border-slate-100">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-[3px]">
                        {t("coAuthors", { count: item.coAuthors.length })}
                      </p>
                    </div>
